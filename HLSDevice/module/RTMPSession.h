...
namespace ScheduleServer
{
	...
	{
	public:
		CRTMPSession() : _rtmp(NULL), _video_packet(NULL), _audio_packet(NULL)
		{
	...
	public:
		...
		{
			/*AudioSpecificConfig* spec = reinterpret_cast<AudioSpecificConfigPtr>(data);

			::memset(spec, 0, sizeof(AudioSpecificConfig));
			...
			spec->channel_num = channel_num;
			spec->tail = 0;

			TRACE("\nAAC SPEC<%x %x>", data[0], data[1]);*/
	...
	private:
		...
		unsigned char _video_packet_buf[65536 * 2];

...
int CRTMPSession::connect(char* url)
{
	...
	if(true)
	{
		...
		pack.m_nBodySize = 4;
		int nVal = 128000;//128000;//65536;//希望指定的大小
...
{
	{
		...
		int h = 0;
		int fps = 0;
		h264_decode_sps(sps, sps_size, w, h, fps);
		TRACE("\n<SPS %d %d %d", w, h, fps);
	}
	...
	memset(_video_packet, 0, RTMP_HEAD_SIZE);

	_video_packet->m_body = (char*)_video_packet + RTMP_HEAD_SIZE;
	body = (unsigned char*)_video_packet->m_body;
	...
	_video_packet->m_packetType = RTMP_PACKET_TYPE_VIDEO;
	_video_packet->m_nBodySize = i;
	_video_packet->m_nChannel = 0x04;
	_video_packet->m_nTimeStamp = 0;
	_video_packet->m_hasAbsTimestamp = 0;
	_video_packet->m_headerType = RTMP_PACKET_SIZE_MEDIUM;
	_video_packet->m_nInfoField2 = _rtmp->m_stream_id;

	{
		...
		{
			return RTMP_SendPacket(_rtmp, _video_packet, FALSE);
		}
	}
...
int CRTMPSession::send_aac_spec(const unsigned int type, const unsigned int sample_rate, const unsigned int channel_num)
{
...

	{
		...
		if(RTMP_IsConnected(_rtmp))
		{
			return RTMP_SendPacket(_rtmp, _audio_packet, FALSE);
		}
	}
...
int CRTMPSession::send_video_packet(unsigned char* data, unsigned long length, NAL_TYPE type, unsigned long time_offset)
{
	...
	//TRACE("\nF<%d %d %d %d> ", data[0], data[1], data[2], data[3]);
	/*去掉帧界定符*/
	...
	_video_packet->m_hasAbsTimestamp = 0;
	_video_packet->m_packetType = RTMP_PACKET_TYPE_VIDEO;
	_video_packet->m_nBodySize = length + 9;
	_video_packet->m_nInfoField2 = _rtmp->m_stream_id;
	_video_packet->m_nChannel = 0x04;
	_video_packet->m_headerType = RTMP_PACKET_SIZE_LARGE;

	TRACE("\n--------- <T %d>", timeoffset - t);
	_video_packet->m_nTimeStamp = timeoffset;
	t = timeoffset;
